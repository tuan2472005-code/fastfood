...
<body>
	...
	<div class="container mt-4">
		...
		<div class="row mb-4">
			<div class="col-12">
				...
				<p class="text-muted">Khám phá các món ăn ngon tại Fast Food</p>
			</div>
		</div>
		...
		<div class="filter-section">
			<div class="row">
				<div class="col-md-6">
					<div class="d-flex">
						<input type="text" id="instantSearch" class="form-control me-2" name="search"
							placeholder="Tìm kiếm sản phẩm..." value="${searchQuery}">
				...
				<div class="col-md-6">
					<div class="d-flex">
						<select id="categoryFilter" name="categoryId" class="form-select me-2">
								<option value="">Tất cả danh mục</option>
								<c:forEach var="category" items="${categories}">
									<option value="${category.id}"
										${selectedCategoryId == category.id ? 'selected' : ''}>${category.name}</option>
								</c:forEach>
							</select> <a href="${pageContext.request.contextPath}/products"
								class="btn btn-outline-secondary"> <i class="fas fa-times"></i>
								Xóa bộ lọc
							</a>
						</div>
				</div>
			...
			<c:choose>
				<c:when test="${empty products}">
					<div class="col-12 text-center py-5">
						...
						<p class="text-muted">Hãy thử tìm kiếm với từ khóa khác hoặc
							xem tất cả sản phẩm</p>
							...
							<div class="card product-card h-100">
								...
								<div class="card-body d-flex flex-column">
									...
									<div class="price-section mb-3">
										<c:choose>
											<c:when
												test="${product.discountPrice != null && product.discountPrice > 0}">
												<span class="price-text"> <fmt:formatNumber
														value="${product.discountPrice}" type="number"
														groupingUsed="true" /> đ
												</span>
												<br>
												<span class="discount-price"> <fmt:formatNumber
			...
			<div class="row mt-4">
				<div class="col-12">
					<div class="alert alert-info">
						<i class="fas fa-info-circle me-2"></i> Hiển thị
	...
	<!-- Footer -->
	<footer class="text-white mt-5 py-4" style="background-color: #ff6b35;">
		<div class="container">
			<div class="row">
				<div class="col-md-4">
					<h5>Fast Food</h5>
				...
				<div class="col-md-4">
					...
					<ul class="list-unstyled">
						...
						<li><a href="${pageContext.request.contextPath}/products"
							class="text-white">Sản phẩm</a></li>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function performSearch() {
            ...
            // Sử dụng phép cộng chuỗi truyền thống để tránh xung đột với JSP EL
            const url = contextPath + '/api/products/search?query=' + encodeURIComponent(query) + '&categoryId=' + categoryId;
        ...
        function renderProducts(products) {
            if (products.length === 0) {
                productsGrid.innerHTML = 
                    '<div class="col-12 text-center py-5">' +
                        ...
                        '<a href="' + contextPath + '/products" class="btn btn-primary">Xem tất cả sản phẩm</a>' +
                    '</div>';
                return;
            }
            ...
            products.forEach(product => {
                const imgPath = (product.imageUrl && (product.imageUrl.startsWith('http') || product.imageUrl.startsWith('https'))) 
                    ? product.imageUrl 
                    : contextPath + '/' + product.imageUrl;
                
                const formattedPrice = new Intl.NumberFormat('vi-VN').format(product.price);
                const formattedDiscount = product.discountPrice ? new Intl.NumberFormat('vi-VN').format(product.discountPrice) : null;
                
                let ratingHtml = '';
                for (let i = 1; i <= 5; i++) {
                    if (i <= product.averageRating) {
                        ratingHtml += '<i class="fas fa-star star"></i>';
                    } else if (i - 0.5 <= product.averageRating) {
                        ratingHtml += '<i class="fas fa-star-half-alt star"></i>';
                    } else {
                        ratingHtml += '<i class="far fa-star star empty"></i>';
                    }
                }
                    ...
                    '<div class="col-lg-3 col-md-4 col-sm-6 mb-4">' +
                        '<div class="card product-card h-100">' +
                            ...
                            '<div class="card-body d-flex flex-column">' +
                                '<h5 class="card-title">' + product.name + '</h5>' +
                                '<div class="mb-2">' +
                                    '<span class="text-muted small"><i class="fas fa-shopping-bag me-1"></i>Đã bán ' + (product.totalSold || 0) + '</span>' +
                                '</div>' +
                                '<p class="card-text flex-grow-1 text-truncate" style="max-height: 3rem;">' + (product.description || '') + '</p>' +
                                
                                '<div class="star-rating">' +
                                    '<div class="stars">' + ratingHtml + '</div>' +
                                    '<span class="rating-text">' + (product.reviewCount > 0 ? product.averageRating.toFixed(1) + ' (' + product.reviewCount + ')' : 'Chưa có đánh giá') + '</span>' +
                                '</div>' +
                ...
                if (formattedDiscount) {
                    html += 
                        '<span class="price-text">' + formattedDiscount + ' đ</span><br>' +
                        '<span class="discount-price">' + formattedPrice + ' đ</span>';
                } else {
                    html += '<span class="price-text">' + formattedPrice + ' đ</span>';
                }

                html += 
                                '</div>' +
                                '<div class="d-grid gap-2">' +
                                    '<a href="' + contextPath + '/product?id=' + product.id + '" class="btn btn-outline-primary btn-sm">' +
                                        '<i class="fas fa-eye me-1"></i>Chi tiết' +
                                    '</a>' +
                                    '<a href="' + contextPath + '/cart/add?productId=' + product.id + '&quantity=1" class="btn btn-primary btn-sm">' +
